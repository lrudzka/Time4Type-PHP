<?php
    ...
    if (isset($_POST['email']))
    {
        ...
        if ($emailOk)
        {
            $queryEmail = $db->prepare('UPDATE types_users set email=:email WHERE login="'.$_SESSION['userLoggedIn'].'"');
    ...
    if (isset($_POST['pwdOld']))
    {
        ...
        $userPwdQuery = $db->prepare('SELECT login, password FROM types_users WHERE login="'.$_SESSION['userLoggedIn'].'"');
        ...
        if ($user && password_verify($password, $user['password']))
        {
            ...
            if ($newPasswordOk)
            {
                ...
                $queryPwd = $db->prepare('UPDATE types_users set password=:password WHERE login="'.$_SESSION['userLoggedIn'].'"');
